/*
  ...
  1. New Tables
    - `email_logs`: Tracks email sending attempts and delivery status
      - Includes order_id (references orders)
      - Records recipient, subject, status
      - Logs errors and SMTP responses
      - Tracks attempt counts and delivery times

  2. New Functions
    - `record_email_attempt`: Records details of email send attempts
    - `update_order_email_status`: Updates order table with email status

  3. Table Changes
    - Add email tracking columns to orders table
      - `emails_sent` (boolean): Whether emails were successfully sent
      - `email_error` (text): Any error that occurred during sending

  4. Security
    - Enable RLS on all tables
    - Add policies for authenticated and service role access
*/
